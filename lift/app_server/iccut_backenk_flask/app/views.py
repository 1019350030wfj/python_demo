...
def register():
    try:
        ...
        user = User.query.filter_by(email=email).first()
        if user != None:
            return formattingData(code=-1, msg="Sorry, this email has already been registered!", data=[])
...
@app.route('/api/newslist', method=['POST', 'GET'])
def newsList():
    try:
...
@app.route('/api/videos', method=['POST', 'GET'])
def videoList():
    try:
        index = request.args.get('index')
        ...
        if index == None or category == None:
            return formattingData(code=-1, msg='Sorry, request parameters are missing!', data=[])

        cateMap = defCat[category]
        if mark == None:
            result = Video.query.filter(Video.leve1 == cateMap).offset(int(pageCount) * int(index)).limit(
                pageCount).all()
        else:
            result = Video.query.filter(Video.leve1 == cateMap).filter(Video.leve2 == str(mark)).offset(
                int(pageCount) * int(index)).limit(pageCount).all()
...
def historyList():
    ...
    try:
        ...
        return formattingData(code=-1, msg='Fetch history fail.', data=[])
